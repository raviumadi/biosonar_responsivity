classdef arrayDataAnalyzerwithBuzzManual
    properties
        ...
        C                   % Speed of sound
        BuzzCallWindows     % Start:End of each call manually selected
    end

    methods
        % Constructor
        function obj = arrayDataAnalyzerwithBuzzManual(wavFileName, outputDir)
            % Load audio data
            ...
            if nargin < 2 || isempty(outputDir)
                outputDir = fullfile(filePath, [fileBase '_Analysis']);
            end
            ...
            obj.Segments = struct('Begin', [], 'End', [], 'Calls', [], 'ValidatedCalls', [], ...
                'Timestamps', [], 'BuzzCallWindows', [], 'CallRate', [],  'CallDurations', [], ...
        ...
        function obj = selectSegments(obj)
            ...
            while true
                ...
                % Ask if this is a buzz segment
                typeChoice = questdlg('Is this a buzz segment?', ...
                    'Segment Type', 'Buzz', 'Regular', 'Regular');
                obj.Segments(segmentIndex).Type = typeChoice;
                ...
                choice = questdlg('Select another segment?', 'Continue?', 'Yes', 'No', 'Yes');
                if strcmp(choice, 'No')
                    break;
                end
            end
            close;
                ...
                % Manual Click-Based Call Selection (Replaces Threshold Detection)
        function obj = setThresholdDetectCalls(obj)
            for i = 2:length(obj.Segments)
                ...
                figure;
                plot(segmentData);
                title(sprintf('Manually Select Calls for Segment %d', i-1));
                xlabel('Sample Number');
                ylabel('Amplitude');
                hold on;
                done = false;
                ...
                while ~done
                    fprintf('Select start and end of call #%d\n', j);
                    k = ginput(2);
                    if size(k,1) < 2
                        warning('Selection incomplete, skipping.');
                        break;
                    end
                    beginCall = round(k(1,1));
                    endCall = round(k(2,1));
                    if beginCall < 1 || endCall > size(segmentData,1) || beginCall >= endCall
                        warning('Invalid call bounds, try again.');
                        continue;
                    end

                    callIdx = callIdx + 1;
                    obj.BuzzCallWindows(callIdx, :) = [beginCall, endCall];
                    callWindows(callIdx,:) = [beginCall, endCall];
                    xline(beginCall, 'g');
                    xline(endCall, 'r');
                    drawnow;

                    choice = questdlg('Select another call?', 'Continue?', 'Yes', 'No', 'Yes');
                    if strcmp(choice, 'No')
                        done = true;
                    end
                    j = j + 1;
                ...
                obj.Segments(i).Calls = mean(callWindows,2); % store midpoint of calls
                segmentDir = fullfile(obj.OutputDir, sprintf('segment_%02d', i-1));
                for j = 1:size(callWindows,1)
                    callWindow = callWindows(j,1):callWindows(j,2);
        ...
        function obj = validateCalls(obj)
            for i = 2:length(obj.Segments)
                ...
                for j = 1:length(calls)
                    % callIdx = calls(j);
                    callWindow = obj.BuzzCallWindows(j,1):obj.BuzzCallWindows(j,2);
                    callData = segmentData(callWindow,:);
        ...
        function obj = analyzeCalls(obj)
            for i = 2:length(obj.Segments)
                ...
                for j = 1:length(callFiles)
                    ...
                    obj.Segments(i).BuzzCallWindows = [obj.BuzzCallWindows];
                end
            end
        ...
        function obj = createAndAppendSegmentsTable(obj, tablePath)
            ...
            if isfile(tablePath)
                ...
                loadedData = load(tablePath, 'AnalysisTable_Buzz');
                if isfield(loadedData, 'AnalysisTable_Buzz')
                    existingTable = loadedData.AnalysisTable_Buzz;
            ...
            AnalysisTable_Buzz = updatedTable; %#ok<NASGU>
            save(tablePath, 'AnalysisTable_Buzz');
