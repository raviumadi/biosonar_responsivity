classdef arrayDataAnalyzerwithBuzz
    properties
    ...
    methods
        % Constructor
        function obj = arrayDataAnalyzerwithBuzz(wavFileName, outputDir)
            % Load audio data
        ...
        function obj = setThresholdDetectCalls(obj)
            for i = 2:length(obj.Segments)
                ...
                while ~done
                    ...
                    % xline(callWindows(:,1), 'g')
                    % xline(callWindows(:,2), 'r')
                    % hold off;
                    ...
                    if strcmp(choice, 'Yes')
                        % Find all windows for each call
                        for j = 1 : length(calls)
                            % Get Call Window size by ginput(2), and use that to
                            % detect the calls. Repeat until user is satisfied.
                            k = ginput(2);
                            obj.BuzzCallWindows(j,1) = round(k(1,1)); 
                            obj.BuzzCallWindows(j,2) = round(k(2,1));
                            xline( obj.BuzzCallWindows(j,1), 'g')
                            xline( obj.BuzzCallWindows(j,2), 'r')
                            drawnow;
                        end
